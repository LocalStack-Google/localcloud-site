...
The BigQuery emulator dependency reports `~96%` SQL coverage, `818` passing tests, `190` audited features, and `175+` mapped functions. It covers common DQL, DDL, DML, scripting, stored procedures, external tables, `11` INFORMATION_SCHEMA views, REST APIs, and simplified gRPC Storage API paths. BQML, AEAD encryption, security policy enforcement, and full geography parity are not complete; geography is partial with `12` working `ST_*` functions.
